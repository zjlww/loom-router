...
pub struct Provider {
    ...
    pub id: String,
    /// Human label shown in agent model pickers and diagnostics.
    pub name: String,
    pub protocol: ProviderProtocol,
    /// Base URL up to (and including) `/v1` or equivalent.
    pub base_url: String,
    /// API key. Stored only locally and never logged.
...
pub struct ProviderKey {
    ...
    #[serde(default)]
    pub enabled: bool,
    /// Stored only locally and never logged.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub api_key: Option<String>,
...
impl AppConfig {
    ...
    /// the display name. Normalize it on read so the stale label is not
    /// surfaced again.
