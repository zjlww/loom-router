...
pub struct CodexStatus {
    ...
    /// dropped the trailing comment. `strip_managed_block` recovers from this
    /// by ownership, and status consumers should surface the degraded state.
    ...
    /// Whether automatic integration refresh is enabled.
...
pub fn status(config: &AppConfig) -> CodexStatus {
    ...
    // `codex doctor` is a subprocess with a 10s ceiling on every status read.
    // There is nothing to validate until the integration has published a
    // catalog, so skip the probe until then.
...
///
/// A launchd or systemd user service can inherit a minimal PATH containing no
/// package manager's bin directory. Codex installs into `~/.local/bin`,
/// `/opt/homebrew/bin`, `~/.bun/bin` and friends, so probing PATH alone is
/// insufficient for background starts. A missing CLI means no native catalog,
/// which means no merged catalog.
///
/// The lookup is cached because the login-shell probe spawns a shell.
...
fn resolve_codex_bin() -> Option<String> {
    ...
    //    honour a PATH they set in their own rc files.
    #[cfg(unix)]
...
/// model pickers so agents can pin a real Codex model instead of only
