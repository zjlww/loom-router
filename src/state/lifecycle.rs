...
use super::{
    fetch_balance, model_discovery, AppConfig, AppState, ProviderBalance, ServerHandle,
    ServerStatus, NEXT_SERVER_ID,
};
use crate::codex;
...
impl AppState {
    ...
    pub async fn save_provider(&self, mut provider: crate::config::Provider) -> anyhow::Result<()> {
        ...
        for key in &mut provider.keys {
            ...
            // An empty key on save means "keep the existing one"; never
            // overwrite a stored credential with an empty value.
        ...
        // context window and fast-mode participation so consumers never see a
        // guess, and re-seed models that exist in the catalog but were dropped
        // (e.g. a stale edit that listed a subset).
    ...
    pub async fn codex_apply(&self) -> anyhow::Result<()> {
        ...
        // describes the pre-apply state and would remain stale for the rest of
        // its TTL.
    ...
    /// reading the state first and locking after lets two concurrent callers
    /// both see "off" and makes one transition disappear.
    ...
    pub async fn set_sleep_prevention(
        ...
        // Roll the in-memory field back on a write failure, or callers observe
        // a mode the wake thread was never told about.
    ...
    pub async fn provider_balances(&self) -> Vec<ProviderBalance> {
        ...
        let probes: Vec<_> = cfg
            ...
            .flat_map(|p| {
                ...
                // provider-level row: the result reports the same failure the
                // proxy would instead of silently omitting the provider.
