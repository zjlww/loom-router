...
// why: Linux desktop distributions use a user-scoped systemd manager for
// background services, which has no launchd equivalent. The unit remains
// per-user so credentials and Codex configuration stay in the operator's home.
#[cfg(target_os = "linux")]
fn platform_install(binary: &Path) -> Result<()> {
